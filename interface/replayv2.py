...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QVBoxLayout, QHBoxLayout, QWidget, QPushButton, QLabel, 
                             QLineEdit, QFileDialog, QSlider, QMessageBox, QFormLayout, QGridLayout, QGroupBox)
...
class IncrementalLearningTool(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.save_button = QPushButton('Save New Dataset and YAML')  # 创建“保存新数据集和YAML”按钮
        self.save_button.clicked.connect(self.save_new_dataset)  # 连接按钮点击事件到保存新数据集方法
        button_layout.addWidget(self.process_button)  # 将“处理数据”按钮添加到布局
        button_layout.addWidget(self.save_button)  # 将“保存新数据集和YAML”按钮添加到布局
    ...
    def add_source(self):
        ...
        if path:
            ...
            # 添加源数据集输入框和滑动条到表单布局
            self.source_layout.addRow(source_input, percentage_slider)
    ...
    def process_data(self):
        ...
        new_dataset_path = os.path.join(os.path.dirname(self.target_path), "new_dataset")  # 创建新数据集路径
    ...
    def save_new_dataset(self):
        new_dataset_path = os.path.join(os.path.dirname(self.target_path), "new_dataset")  # 新数据集路径
