...
from PyQt5.QtWidgets import (QApplication, QWidget, QGridLayout, QLabel, QLineEdit, QPushButton, QFileDialog,
                             QMessageBox, QTextEdit, QTabWidget, QVBoxLayout, QCheckBox)
...
class YOLOv9Interface(QWidget):
    ...
    def initUI(self):
        ...
        self.train_tab_layout.addWidget(self.output_display, 7, 0, 1, 3)
        ...
        self.detect_output_display = QTextEdit(self)
        ...
        # Output display for detecting
        self.detect_output_display_dir = QTextEdit(self)
    ...
    def save_detection(self):
        if hasattr(self, 'result_image_path'):
            options = QFileDialog.Options()
            save_path, _ = QFileDialog.getSaveFileName(self, "Save Detection", "", "Images (*.png *.xpm *.jpg);;All Files (*)", options=options)
            if save_path:
                try:
                    shutil.copy(self.result_image_path, save_path)
                    QMessageBox.information(self, "Save Successful", f"Image saved to {save_path}")
                except Exception as e:
                    QMessageBox.critical(self, "Save Failed", f"Failed to save image: {str(e)}")
        else:
        ...
        if hasattr(self, 'result_directory_path'):
            ...
            if save_dir:
                try:
                    ...
                    QMessageBox.information(self, "Save Successful", f"All images saved to {save_dir}")
                except Exception as e:
                    QMessageBox.critical(self, "Save Failed", f"Failed to save images: {str(e)}")
        else:
    ...
    def display_detection_results(self, result_dir):
        ...
        if os.path.exists(labels_dir):
            ...
            self.detect_output_display.append(result_text)
        else:
            self.detect_output_display.append("No labels directory found in result directory.")
