...
import yaml
from PyQt5.QtWidgets import (QWidget, QGroupBox, QLineEdit, QPushButton, QVBoxLayout, QFileDialog, QProgressBar, QCheckBox, QHBoxLayout, QMessageBox, QSpinBox, QDialog, QFormLayout, QDialogButtonBox, QLabel)
from PyQt5.QtCore import Qt
from utils import convert_float32, bbox_to_xywh
            ...
            if filename.endswith('.json'):
                ...
                with open(json_path, 'r', encoding='utf-8') as f:
                    ...
                    for shape in data['shapes']:
                        ...
                        class_index[shape['label']] = shape.get('class_id')
            ...
            if dialog.exec_():
                for label in class_labels:
                    if label in class_index:
                        dialog.class_info.append((label, class_index[label]))
                    else:
                        new_id = max(class_index.values()) + 1 if class_index else 0
                        dialog.class_info.append((label, new_id))
                        class_index[label] = new_id
