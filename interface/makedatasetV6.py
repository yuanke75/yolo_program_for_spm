...
#     def __init__(self, index_folder="label_indices", index_file="label_index.json"):
#         self.index_folder = index_folder
#         self.index_file_path = os.path.join(index_folder, index_file)
...
#         super().__init__()
#         self.initUI()
#         self.label_index_manager = LabelIndexManager()
...
#             if dialog.exec_():
#                 for label, _ in dialog.class_info:
#                     class_id = self.label_index_manager.assign_class_id(label)
#                     self.class_info.append((label, class_id))
...
#             class_info = [(label, self.label_index_manager.assign_class_id(label)) for label in class_labels]
...
class ExpertApp(QWidget):
    ...
    def save_augmented_image(self, image_aug, bbs_aug, squad_box, image_name, image_path, jpegimages_path, worktxt_path):
        ...
        new_shapes = []
        for i, bb_aug in enumerate(bbs_aug.bounding_boxes):
            x1_new, y1_new, x2_new, y2_new = bb_aug.x1, bb_aug.y1, bb_aug.x2, bb_aug.y2
            new_shapes.append({
                'label': squad_box['shapes'][i]['label'],
                'points': [[x1_new, y1_new], [x2_new, y2_new]],
                'group_id': squad_box['shapes'][i].get('group_id'),
    ...
    def enter_class_info(self):
        ...
        with open(self.label_index_path, 'r', encoding='utf-8') as index_file:
            label_index = json.load(index_file)
        
        class_info = []
        for label in class_labels:
            found = False
            for class_id, class_str in label_index.items():
                if ':' in class_str:
                    class_name = class_str.split(':')[1]
                    if class_name == label:
                        class_info.append((label, int(class_id)))
                        found = True
                        print(label)
                        break
            if not found:
                QMessageBox.warning(self, 'Label Not Found', f'Label "{label}" not found in index.')
