...
class BasicApp(QWidget):
    ...
    def browse_filename(self):
        ...
        if filename:
            ...
            # Set output path automatically to the directory containing the JSON file
            self.outputpath = os.path.dirname(filename)

    def make_dataset(self):
        QMessageBox.information(
            self, 'Please Wait', 'Please wait, creating the dataset...')
        ...
        if not json_file or not os.path.exists(json_file):
            QMessageBox.warning(
                self, 'Error', 'Please select a valid JSON file.')
            return

        # Set output path to the directory containing the JSON file
        output_path = os.path.dirname(json_file)
        self.outputpath = output_path

        # Update expert app paths
        self.expert_app.outputpath_input.setText(output_path)
        self.expert_app.json_folder_input.setText(output_path)
        ...
        # Automatically enter class info
        self.expert_app.json_folder_path = output_path
        self.expert_app.enter_class_info()

        if not self.expert_app.class_info:
            QMessageBox.warning(
                self, 'Error', 'No class information was found or matched.')
            return
