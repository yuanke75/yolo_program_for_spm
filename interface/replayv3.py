...
class IncrementalLearningTool(QMainWindow):
    ...
    def process_data(self):
        ...
        for i, source_path in enumerate(self.source_paths):
            source_dir = os.path.join(source_path, 'voc', 'ImageSets')  # 源数据集的路径
            percentage = self.source_percentages[i]  # 获取该源数据集的百分比
            print(f"Processing source: {source_dir}")  # 调试信息
        ...
        target_dir = os.path.join(self.target_path, 'voc', 'ImageSets')
        for file_name in source_files:
            ...
            # 将合并的行写入新的数据集文件
            new_file_path = os.path.join(new_dataset_path, 'voc', 'ImageSets', file_name)
...
train: '{new_dataset_path.replace(os.sep, '/')}/voc/ImageSets/img_train.txt'
val: '{new_dataset_path.replace(os.sep, '/')}/voc/ImageSets/img_valid.txt'
test: '{new_dataset_path.replace(os.sep, '/')}/voc/ImageSets/img_test.txt'
# Classes
names:
"""
        for i in range(80):
