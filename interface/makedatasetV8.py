...
#                     if label in item:
#                         class_info.append((label, item[0]))
#                         matched = True
#                         break
#                 if not matched:
...
class ExpertApp(QWidget):
    ...
    def start_augmentation(self):
        ...
        seq = iaa.Sequential([iaa.Affine(rotate=rotation_list)] + augments, random_order=True)
        ...
        for theta in rotation_list:
            ...
            bbs = BoundingBoxesOnImage(bbs_list, shape=image.shape)
            rot = iaa.Affine(rotate=theta)
            image_aug, bbs_aug = rot(image=image, bounding_boxes=bbs)

            # Crop the black borders
            image_aug, bbs_aug = self.crop_to_non_black(image_aug, bbs_aug)
    ...
    def crop_to_non_black(self, image, bbs):
        mask = image.sum(axis=2) > 0  # Non-black pixel mask
        ...
        if coords.any():
            ...
            bbs_cropped = bbs.shift(x=-y0, y=-x0)
            return image_cropped, bbs_cropped
