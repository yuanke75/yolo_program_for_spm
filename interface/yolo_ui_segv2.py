...
class YOLOv9Interface_seg(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()
    ...
    def start_detecting(self):
        ...
        command = f"python segment/predict.py --source {image} --weights {weights} --conf-thres {conf_thres}"
    ...
    def start_detecting_dir(self):
        ...
        command = f"python segment/predict.py --source {directory} --weights {weights} --conf-thres {conf_thres}"
        command += f" --hide-labels {'True' if hide_labels else 'False'}"
        command += f" --hide-conf {'True' if hide_conf else 'False'}"
        print(command)
    ...
    def on_ready_read_standard_output(self, output_display):
        ...
        while self.process.canReadLine():
            # 逐行读取输出
            output = self.process.readLine().data().decode('utf-8').strip()
            # 将读取的输出添加到输出显示区域
