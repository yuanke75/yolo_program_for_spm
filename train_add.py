...
from utils.loss_tal import ComputeLoss,ewc_loss
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    #         weights = attempt_download(weights)  # download if not found locally
    #     ckpt = torch.load(weights, map_location='cpu')  # load checkpoint to CPU to avoid CUDA memory leak
    #     model = Model(cfg or ckpt.get('model').yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
    #     exclude = ['anchor'] if (cfg or hyp.get('anchors')) and not resume else []  # exclude keys
        
    #     # 检查ckpt中是否直接存储了模型状态字典，而不是模型对象
    #     if 'state_dict' in ckpt:
    #         state_dict = ckpt['state_dict']  # 如果状态字典直接在根下
    #     elif 'model' in ckpt:
    #         state_dict = ckpt['model']  # 兼容旧的格式，其中模型状态字典在'model'键下
    #     else:
    #         raise KeyError("不支持的检查点格式")

    #     # 在将状态字典加载到模型之前，无需转换为float，因为状态字典本身不支持此操作
    ...
    fisher_information = torch.load('D:/code/yolov9-main/fisher3/fisher_information_task_train.pt')
    lambda_ewc = hyp['lambda_ewc'] # 可根据需要调整
    model_parameters_task = torch.load('D:/code/yolov9-main/fisher3/ewc_model_task_train.pt')
    # model_parameters_task = torch.load('runs/train/exp39/weights/best.pt')
    
    print(type(model_parameters_task))
    print(list(model_parameters_task.keys())[:5]) 
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                # 这里添加EWC损失
                ewc_loss_val = ewc_loss(model, fisher_information, model_parameters_task, lambda_ewc)
                loss11 = loss + ewc_loss_val

                if RANK != -1:
                    loss11 *= WORLD_SIZE  # gradient averaged between devices in DDP mode
                if opt.quad:
                    loss11 *= 4.
                # print(f"Loss: {loss.item()}, EWC Loss: {ewc_loss_val.item()}, Total Loss (Loss + EWC Loss): {loss11.item()}")
            # Backward
            ...
            #     # loss11 = loss + ewc_loss_val
            #     # 计算总损失并进行梯度缩放
            #     total_loss = loss + ewc_loss_val * lambda_ewc
            ...
            #         loss *= 4.
            #     # print(f"Loss: {loss.item()}, EWC Loss: {ewc_loss_val.item()}, Total Loss (Loss + EWC Loss): {loss11.item()}")
            #     print(f"Loss: {loss.item()}, EWC Loss: {ewc_loss_val.item()}")
            # # Backward
            ...
            #     scaler.scale(ewc_loss_val).backward()
            #     scaler.step(optimizer)  # optimizer.step
            #     scaler.update()
            #     optimizer.zero_grad()
            #     if ema:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/moleculeleft.yaml', help='dataset.yaml path')
    parser.add_argument('--hyp', type=str, default=ROOT / 'temp_hyp/list11/hyp_trial_13.yaml', help='hyperparameters path')
